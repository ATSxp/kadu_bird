#include "../include/s_intro.hpp"
#include "../include/global.hpp"
#include <vector>

#include "map_intro01.h"

namespace Intro {
#define INTRO(i)                                                               \
  { i##Bitmap, (const COLOR *)i##Pal }
#define MAX_T_SLIDE 0x0400

static FIXED evy{0x080}, evdy{0x00}, t_slide{MAX_T_SLIDE};
static u8 count{0}, old_count{0};

typedef struct {
  const void *bitmap;
  const COLOR *pal;
} IntroTemplate;

const std::vector<IntroTemplate> intro{
    INTRO(map_intro01),
};

void reloadSlide();

void init() {
  T_setMode(4);
  T_enableBg(2);

  REG_BLDCNT = BLD_BG2 | BLD_WHITE;

  reloadSlide();
}

void update() {
  if (t_slide <= 0x00) {
    t_slide = MAX_T_SLIDE;
    count++;
  } else if (evy <= 0x00)
    t_slide -= 0x010;

  if (old_count != count) {
    evdy = 0x02;
    reloadSlide(); }

  if (count >= (int)intro.size() && evy >= 0x080)
    Scener::set(Global::s_menu);

  evy = clamp(evy += evdy, 0x00, 0x081);
  REG_BLDY = BLDY_BUILD(evy >> 3);
}

void end() {
  RegisterRamReset(RESET_PALETTE);
  RegisterRamReset(RESET_VRAM);
  REG_BLDY = 0;
}

void reloadSlide() {
  if (count >= (u8)intro.size())
    return;
  if (evy >= 0x080) {
    old_count = count;
    LZ77UnCompVram(intro[count].bitmap, m4_mem);
    LZ77UnCompVram(intro[count].pal, pal_bg_mem);
    evdy = -0x01;
  }
}
} // namespace Intro

namespace Global {
Scener::Scene s_intro = {Intro::init, Intro::update, Intro::end};
}
